import { f as current_component, s as sanitize_props, p as push, g as spread_attributes, b as pop } from './global.d_CfyAfVtb.mjs';
...
function Icon($$payload, $$props) {
	...
	// State
	const state = { // Last icon name
	name: '', // Loading status
	loading: null, // Destroyed status
	destroyed: false };
	// Mounted status
	let mounted = false;
	// Generated data
	...
	if (data) {
		...
		if (data.svg) {
			$$payload.out += '<!--[-->';
			$$payload.out += `<svg${spread_attributes({ ...data.attributes }, null, undefined, undefined, 3)}>${html(data.body)}</svg>`;
		} else {
			$$payload.out += '<!--[!-->';
